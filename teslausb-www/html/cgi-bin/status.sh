...
external_5v=$(sudo -n vcgencmd pmic_read_adc EXT5V_V 2>/dev/null | sed -n 's/.*=//p'); external_5v=${external_5v:-N/A}
rtc_batt_v=$(sudo -n vcgencmd pmic_read_adc BATT_V 2>/dev/null | sed -n 's/.*=//p'); rtc_batt_v=${rtc_batt_v:-N/A}
